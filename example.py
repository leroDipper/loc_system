...
def main():
    # Load map
    print("Loading map...")
    xyz_world, map_descriptors = MapLoader.load_map('map/colmap_map.npz')
    print(f"Loaded {len(xyz_world)} 3D points")
    
    # Load camera intrinsics
    K = MapLoader.load_camera_intrinsics('improved_office_dataset/ground_truth_poses.json')

    gt_positions = load_colmap_ground_truth('map/project_files/images.txt')
    ...
    result, error = localiser.localise('improved_office_dataset/left/frame_0010.jpg')
    ...
    images = [
        'improved_office_dataset/left/frame_0001.jpg',
        'improved_office_dataset/left/frame_0010.jpg',
        'improved_office_dataset/left/frame_0060.jpg',
    ]
    ...
    for img_path, (result, error) in zip(images, results):
        ...
        if result:
            print(f"{img_name}: Position {result['position']}")
        else:
