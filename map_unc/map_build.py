...
class MapBuilder:
    ...
    def build_map_database(self, map_files, dataset_path, descriptors_path, save_to=None, train_images=None):
        ...
        map_3d_points = []
        map_descriptors = []
        map_track_lengths = []  # NEW: Store track lengths
        map_ba_errors = []  # NEW: Store BA errors
        ...
        for point in points:
            ...
            if first_kp_idx < len(descriptors):
                ...
                map_track_lengths.append(point['track_length'])  # NEW
                map_ba_errors.append(point['ba_error'])  # NEW

        map_3d_points = np.array(map_3d_points, dtype=np.float32)
        map_descriptors = np.array(map_descriptors, dtype=np.float32)
        map_track_lengths = np.array(map_track_lengths, dtype=np.int32)  # NEW
        map_ba_errors = np.array(map_ba_errors, dtype=np.float32)  # NEW
    ...
    def save_map(self, npz_path, map_3d_points, map_descriptors, 
                 map_track_lengths, map_ba_errors):
        """Save map with quality metrics."""
        np.savez_compressed(
            npz_path,
            xyz_world=map_3d_points,
            descriptors=map_descriptors,
            track_lengths=map_track_lengths,  # NEW
            ba_errors=map_ba_errors  # NEW
        )
