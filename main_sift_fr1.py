...
if __name__ == "__main__":
    scale, R, t = GroundTruthParams.load_transformation('sift_stuff/tum_fr1/colmap_to_gt_transform.json')
    ...
    MemoryMonitor.print_memory("After loading SIFT")

    test_dataset_path = 'resources/tum_fr1/images'

    # Load existing vocabulary
    vocabulary = 'sift_stuff/tum_fr1/vocabularies/vocab_tree.bin'
    print("Loaded existing vocabulary")

    # Load colmap map
    data = np.load('sift_stuff/tum_fr1/map_databases/tumfr1_map_train.npz')
    ...
    # ===================================================================
    print("\n" + "="*60)
    print("CONTINUOUS LOCALIZATION TEST - TUM FR1 (SIFT)")
    print("="*60)
    ...
    for i, frame_path in enumerate(test_frames):
        ...
        # Convert keypoints to numpy array (x, y coordinates)
        keypoints_np = np.array([kp.pt for kp in keypoints], dtype=np.float32)
        
        # SIFT descriptors are already uint8, no conversion needed
        ...
        for q_idx, (m_idx, dist) in query_to_map.items():
            ...
            matched_2d.append(keypoints_np[q_idx])
    ...
    print("\n" + "="*60)
    print("CONTINUOUS LOCALIZATION RESULTS (SIFT)")
    print("="*60)
    ...
    if len(errors) > 0:
        ...
        np.savez('results/fr1_sift_errors.npz',
