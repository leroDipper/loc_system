...
Test INT4 ONNX model (QOperator format) with existing FP32 map.
"""
...
if __name__ == "__main__":
    ...
    # Load INT4 ONNX model (QOperator format)
    print("Loading INT4 ONNX model (QOperator)...")
    int4_model_path = 'models/xfeat_640x480_int4.onnx'
    session = ort.InferenceSession(int4_model_path, providers=['CPUExecutionProvider'])
    print("✓ INT4 model loaded")

    MemoryMonitor.print_memory("After loading INT4 ONNX model")
    ...
    # ===================================================================
    # CONTINUOUS LOCALIZATION TEST - INT4 queries vs FP32 map
    # ===================================================================
    print("\n" + "="*60)
    print("INT4 QUERIES vs FP32 MAP - TUM FR1")
    print("="*60)
    ...
    print(f"Testing with {len(test_frames)} remaining images (INT4)\n")
    ...
    for i, frame_path in enumerate(test_frames):
        ...
        # Extract features using INT4 ONNX
        ...
        # Match INT4 descriptors against FP32 map
        t_start = time.time()
    ...
    print("\n" + "="*60)
    print("INT4 vs FP32 MAP RESULTS")
    print("="*60)
    ...
    if len(errors) > 0:
        ...
        np.savez('results/fr1_int4_errors.npz',
        ...
        print("✓ Saved errors to results/fr1_int4_errors.npz")
