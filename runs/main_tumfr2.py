...
if __name__ == "__main__":
    ...
    # ===================================================================
    print("\n" + "="*60)
    print("CONTINUOUS LOCALIZATION TEST - TUM FR2")
    print("="*60)
    ...
    for i, frame_path in enumerate(test_frames):
        ...
        if success and len(inliers) >= 6:
            ...
            if median_reproj_error > 5.0:  # pixels
    ...
    print(f"Total failures: {skipped_no_gt + skipped_no_image + skipped_too_few_matches + pnp_failed + rejected_low_inliers + rejected_reproj_error}")
    print(f"Sanity check (should = {len(test_frames)}): {len(errors) + skipped_no_gt + skipped_no_image + skipped_too_few_matches + pnp_failed + rejected_low_inliers + rejected_reproj_error}")
