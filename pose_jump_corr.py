...
def load_alignment(align_path):
    ...
    with open(align_path, 'r') as f:
        for line in f:
            ...
            if len(parts) == 7:
                rows.append({'frame': parts[0], 'rx': float(parts[4]), 'ry': float(parts[5]), 'rz': float(parts[6])})
...
for env in environments:
    ...
    try:
        ...
        total = w_phys + w_proj + w_jump
        w_phys /= total; w_proj /= total; w_jump /= total

        combined = w_phys * normalise(merged['translation_std_total_m']) + w_proj * normalise(merged['projection']) + w_jump * normalise(merged['pose_jump'])

        pearson = np.corrcoef(combined, merged['error_m'])[0, 1]
        spearman = spearmanr(combined, merged['error_m']).statistic

        print(f"\n{env.upper()} (n={len(merged)})")
        print(f"  Pearson combined:  {pearson:+.3f}")
        print(f"  Spearman combined: {spearman:+.3f}")
