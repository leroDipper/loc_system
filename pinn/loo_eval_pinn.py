...
FAILURE_THRESHOLD = {
    env: None for env in ENVIRONMENTS  # filled dynamically below
}

df = pd.read_csv('results/training_data.csv')

for env in ENVIRONMENTS:
    FAILURE_THRESHOLD[env] = df[df['env'] == env]['error_m'].quantile(0.95)
...
for held_out in ENVIRONMENTS:
    ...
    for epoch in range(EPOCHS):
        for X_batch, y_batch, crb_batch in loader:
            opt.zero_grad()
            pinn_loss(model(X_batch), y_batch, crb_batch).backward()
            opt.step()
